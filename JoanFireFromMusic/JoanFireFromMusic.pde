...
boolean bSerialInit = false;

// Cannon state (0x0F is all cannons on, 0x00 is all off)
int iCannonState = 0;
int iOldCannonState = 0;

// The index of the current cannon firing (0-3)
int iCannonIndex = 0;
...
void setup() {
  ...
  size(1000, 500, OPENGL);                                      //Sets up window
  ...
  println("Looking for serial ports:");
...
void draw() {
  ...
  if(iCannonState != iOldCannonState)
  {
	  if(bSerialInit)
	  {
		g_port.write(iCannonState);
	  }
	  iOldCannonState = iCannonState;
  }
